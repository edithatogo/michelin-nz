...
The aggregate country ledger is a repo-local 2026 snapshot derived from a secondary country-ranking table that cites the Michelin restaurant search, then reconciled against a checked-in official Michelin guide-market coverage snapshot. Official guide markets with star awards must map to an aggregate ledger row; official guide markets without star awards, such as Saudi Arabia, are documented in the coverage snapshot but excluded from the star dashboard. New Zealand remains a local benchmark row and is not represented as an official Michelin Guide country.
